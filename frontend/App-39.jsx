...
import { CONTRACTS, FLARE_CHAIN, SONGBIRD_CHAIN, SONGBIRD_CONTRACTS, ABIS, LOCK_TIERS, OG_COLLECTIONS } from './contracts.js';
...
const ToadzFinal = () => {
 ...
 const [marketListings, setMarketListings] = useState([]);
  ...
  const switchToFlare = async () => {
    try {
      await window.ethereum.request({
        ...
        params: [{ chainId: FLARE_CHAIN.chainId }],
      });
  ...
  // Switch to Songbird network
  const switchToSongbird = async () => {
    try {
      await window.ethereum.request({
        method: 'wallet_switchEthereumChain',
        params: [{ chainId: SONGBIRD_CHAIN.chainId }],
      });
      setCurrentNetwork('songbird');
    } catch (switchError) {
      if (switchError.code === 4902) {
        await window.ethereum.request({
          method: 'wallet_addEthereumChain',
          params: [SONGBIRD_CHAIN],
        });
        setCurrentNetwork('songbird');
      }
    }
  ...
  const loadPublicStats = async () => {
    try {
      const rpcProvider = new ethers.JsonRpcProvider(FLARE_CHAIN.rpcUrls[0]);
...
React.useEffect(() => {
  const loadListings = async () => {
    try {
      ...
      const market = new ethers.Contract(SONGBIRD_CONTRACTS.ToadzMarket, [
...
setLockExpired(isExpired);
      ...
      try {
        const songbirdProvider = new ethers.JsonRpcProvider('https://songbird-api.flare.network/ext/C/rpc');
        const songbirdOGVault = new ethers.Contract(SONGBIRD_CONTRACTS.OGVault, ['function lockedCount(address) view returns (uint256)'], songbirdProvider);
        const songbirdMarket = new ethers.Contract(SONGBIRD_CONTRACTS.ToadzMarket, ['function getUserListingCount(address) view returns (uint256)'], songbirdProvider);
      ...
      const songbirdProvider = new ethers.JsonRpcProvider(SONGBIRD_CHAIN.rpcUrls[0]);
      const ogCollectionsData = await Promise.all(
        OG_COLLECTIONS.map(async (col) => {
          try {
            ...
            let lockedTokenIds = [];
            try {
              const ogVault = new ethers.Contract(SONGBIRD_CONTRACTS.OGVault, ABIS.OGVault, songbirdProvider);
...
useEffect(() => {
  ...
  const handleDeposit = async (amount, tier) => {
    ...
    if (chainId !== FLARE_CHAIN.chainId) {
      await switchToFlare();
    }
  ...
  const handleLockOG = async (collection, tokenId) => {
    ...
    const chainId = await window.ethereum.request({ method: 'eth_chainId' });
    if (chainId !== SONGBIRD_CHAIN.chainId) {
    ...
    try {
      ...
      const isApproved = await nftContract.isApprovedForAll(walletAddress, SONGBIRD_CONTRACTS.OGVault);
      if (!isApproved) {
        const approveTx = await nftContract.setApprovalForAll(SONGBIRD_CONTRACTS.OGVault, true, { gasLimit: 500000 });
        await approveTx.wait();
      }
      
      // Lock in OGVault
      const ogVault = new ethers.Contract(SONGBIRD_CONTRACTS.OGVault, ABIS.OGVault, songbirdSigner);
...
// ============ BOOST MARKET SECTION (REPLACES OLD MARKETPLACE) ============
  ...
  const ListModal = () => {
    ...
    const marketAddress = isFlare ? CONTRACTS.ToadzMarket : SONGBIRD_CONTRACTS.ToadzMarket;
